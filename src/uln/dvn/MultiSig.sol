...
abstract contract MultiSig {
    ...
    function verifySignatures(bytes32 _hash, bytes calldata _signatures) public view returns (bool, Errors) {
        ...
        for (uint256 i = 0; i < quorum; i++) {
            ...
            (address currentSigner, ECDSA.RecoverError error, bytes32 signatureLength) =
                ECDSA.tryRecover(messageDigest, signature);

            if (signatureLength.length != 65) return (false, Errors.SignatureLenghtError); // signature lenmust be 65
            if (error != ECDSA.RecoverError.NoError) return (false, Errors.SignatureError); // signature must be valid
